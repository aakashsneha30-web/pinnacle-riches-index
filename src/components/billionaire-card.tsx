...
import {
  ...
  Twitter, Facebook, Instagram, Send, Sparkles, X, Box, Loader2, Copy, Check, Crown, Diamond
} from "lucide-react";
...
function VerifiedBadge({ tier }: { tier: Billionaire["verifiedTier"] }) {
  ...
  const color =
    tier === "pinnacle" ? "text-[oklch(0.7_0.18_230)] glow-text" : "text-gold glow-text";
  return (
...
function ShareMenu({ b, onClose }: { b: Billionaire; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl border-shimmer glass-luxe p-6 shadow-luxe" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-xl glow-text">Share Your Achievement</h3>
        ...
        <div className="grid grid-cols-4 gap-2">
          {items.map(({ icon: Icon, label, href }) => (
            <a key={label} href={href} target="_blank" rel="noreferrer"
              className="flex flex-col items-center gap-1 rounded-xl border-shimmer glass-luxe p-3 text-xs transition hover:bg-secondary hover:text-gold">
        ...
        <button
          ...
          className="mt-4 w-full rounded-xl border-shimmer glass-luxe px-4 py-2 text-sm hover:bg-secondary">
          Copy share link
        </button>
      </div>
    </div>
...
function MagazineModal({ b, onClose }: { b: Billionaire; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4" onClick={onClose}>
      <div className="w-full max-w-2xl rounded-2xl border-shimmer glass-luxe p-6 shadow-luxe" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-2xl glow-text">3D Cover — {b.name}</h3>
          <button onClick={onClose} className="rounded-full p-1 hover:bg-secondary"><X className="size-4" /></button>
        </div>
        <div className="relative aspect-[3/4] overflow-hidden rounded-xl bg-gradient-to-br from-[oklch(0.2_0.03_80)] to-[oklch(0.08_0.01_80)]">
          {loading ? (
            <div className="absolute inset-0 grid place-items-center text-muted-foreground">
              <div className="flex flex-col items-center gap-3">
                <Loader2 className="size-8 animate-spin text-gold glow-text" />
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-between p-8 [transform:perspective(1200px)_rotateY(-12deg)] transition-transform duration-700 hover:[transform:perspective(1200px)_rotateY(0deg)]">
              <div className="w-full text-center">
                <div className="font-serif text-4xl tracking-wider text-gradient-gold glow-text">PINNACLE</div>
...
function ShimmerBorder({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`group relative ${className}`}>
      <div className="absolute inset-0 -z-10 rounded-2xl bg-gradient-to-r from-gold-deep via-gold to-gold-deep opacity-0 blur-xl transition-opacity duration-700 group-hover:opacity-30" />
      <div className="shimmer-gold absolute inset-0 -z-10 rounded-2xl opacity-0 transition-opacity duration-700 group-hover:opacity-100" />
      {children}
    </div>
  );
}
...
export function BillionaireCard({ b, accent = "gold" }: { b: Billionaire; accent?: "gold" | "platinum" }) {
  ...
  const gradientId = `g-${b.rank}-${b.name.replace(/\s/g, "")}`;

  return (
    <article
      className="group relative overflow-hidden rounded-2xl border-shimmer glass-luxe shadow-luxe transition-all duration-500 hover:-translate-y-1.5 hover:shadow-glow"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      <div className="pointer-events-none absolute -right-20 -top-20 size-72 rounded-full bg-gradient-gold opacity-[0.07] blur-3xl transition-opacity duration-700 group-hover:opacity-[0.14]" />
      <div className="pointer-events-none absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-700 group-hover:opacity-100"
        style={{ boxShadow: "inset 0 0 80px oklch(0.85 0.14 85 / 0.06)" }} />
          ...
          <div className="relative">
            {b.image ? (
              <img src={b.image} alt={b.name}
                className="size-28 rounded-2xl object-cover ring-2 ring-gold/40 md:size-32 transition-all duration-500 group-hover:ring-gold/70 group-hover:shadow-glow" />
            ) : (
              <div className={`grid size-28 place-items-center rounded-2xl font-serif text-4xl text-primary-foreground md:size-32 transition-all duration-500 ${
                accent === "gold" ? "bg-gradient-gold" : "bg-secondary text-foreground"
              }`}>{b.name.charAt(0)}</div>
            )}
            <span className="absolute -bottom-2 -right-2 grid size-10 place-items-center rounded-full bg-gradient-gold text-sm font-bold text-primary-foreground shadow-glow float-anim">
              {b.rank}
            </span>
            {b.rank === 1 && (
              <div className="absolute -top-2 -left-2">
                <Crown className="size-6 text-gold glow-text sparkle-anim" />
              </div>
            )}
          </div>
        ...
        <div className="space-y-5">
          <header className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="text-2xl md:text-3xl glow-text">{b.name}</h3>
          ...
          <div className="flex flex-wrap items-baseline gap-x-6 gap-y-2">
            <div>
              ...
              <div className="text-3xl text-gradient-gold font-serif glow-text">{b.netWorth}</div>
            </div>
            {b.pricePerShare && (
              <div>
          ...
          <div className="rounded-xl border-shimmer glass-luxe p-4">
            <div className="mb-2 flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm">
                <Building2 className="size-4 text-gold glow-text" />
                ...
                <AreaChart data={data} margin={{ top: 5, right: 0, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="oklch(0.85 0.14 85)" stopOpacity={hovered ? 0.8 : 0.55} />
                      <stop offset="50%" stopColor="oklch(0.78 0.13 80)" stopOpacity={hovered ? 0.4 : 0.2} />
                      <stop offset="100%" stopColor="oklch(0.85 0.14 85)" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <XAxis dataKey="m" stroke="oklch(0.55 0.02 85)" fontSize={10} tickLine={false} axisLine={false} interval={3} />
                  <YAxis stroke="oklch(0.55 0.02 85)" fontSize={10} tickLine={false} axisLine={false} width={32} />
                  <Tooltip
                    contentStyle={{
                      background: "oklch(0.13 0.02 80 / 0.95)",
                      border: "1px solid oklch(0.85 0.14 85 / 35%)",
                      borderRadius: 12,
                      fontSize: 12,
                      boxShadow: "0 8px 32px oklch(0 0 0 / 60%), 0 0 0 1px oklch(0.85 0.14 85 / 20%)",
                    }}
                    itemStyle={{ color: "oklch(0.85 0.14 85)" }}
                  />
                  <Area
                    type="monotone"
                    dataKey="v"
          ...
          <div>
            ...
            <ul className="grid gap-2 sm:grid-cols-2">
              {b.stakes.map((s) => (
                <li key={s.company} className="flex items-start gap-2 rounded-xl border-shimmer glass-luxe p-3 text-sm transition-all duration-300 hover:shadow-glow">
                  <Sparkles className="mt-0.5 size-4 shrink-0 text-gold glow-text" />
                  <div>
                    <div className="font-medium">{s.company} {s.percent && <span className="text-gold glow-text">· {s.percent}</span>}</div>
          ...
          <div className="flex items-start gap-2 rounded-xl border-shimmer glass-luxe p-3 text-xs text-muted-foreground">
            <BadgeCheck className="mt-0.5 size-4 shrink-0 text-gold glow-text" />
            <div>{b.verifiedBy} · <span className="italic">Source: {b.source}</span></div>
          </div>

          <div className="flex flex-wrap gap-2">
            <button onClick={() => setShareOpen(true)}
              className="inline-flex items-center gap-2 rounded-full bg-gradient-gold px-4 py-2 text-sm font-medium text-primary-foreground transition-all duration-300 hover:shadow-glow hover:brightness-110">
              <Share2 className="size-4" /> Share achievement
            </button>
            <button onClick={() => setMagOpen(true)}
              className="inline-flex items-center gap-2 rounded-full border-shimmer glass-luxe px-4 py-2 text-sm transition-all duration-300 hover:bg-secondary hover:shadow-glow">
  ...
  return (
    <section className="mx-auto max-w-5xl px-6 py-20">
      <div className="mb-8 text-center">
        <div className="text-xs uppercase tracking-[0.3em] text-gold glow-text">Developers</div>
        <h2 className="mt-2 text-4xl glow-text">Public API · Embed the Index</h2>
      ...
      <div className="overflow-hidden rounded-2xl border-shimmer glass-luxe shadow-luxe">
        <div className="flex items-center justify-between border-b border-gold/20 px-4 py-2">
          ...
          <button onClick={() => { navigator.clipboard?.writeText(snippet); setCopied(true); toast.success("API snippet copied"); setTimeout(() => setCopied(false), 1500); }}
            className="inline-flex items-center gap-1.5 rounded-md border-shimmer glass-luxe px-2.5 py-1 text-xs hover:bg-secondary">
            {copied ? <Check className="size-3.5 text-gold glow-text" /> : <Copy className="size-3.5" />} {copied ? "Copied" : "Copy"}
          </button>
        </div>
